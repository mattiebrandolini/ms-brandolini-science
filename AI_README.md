...
1. In `js/toolbar.js`:
   ...
   - The toggle handler is generic — it just sets/removes the attribute.
...
2. In `js/course-viewer.js`, the type rendering is automatic — types are extracted from the CSV `type` column. No code change needed unless you want a custom icon or special rendering.
3. Rebuild to refresh the CSV snapshot.

### Update CSV snapshots (refresh offline data)

Run `cd build && python3 build.py` — it fetches fresh CSVs automatically.
...
```
User visits page
  → Static HTML (generated by build.py from Jinja2 templates)
  → CSS loads (self-hosted fonts via static/fonts/fonts.css)
  → toolbar.js injects theme toggle + accessibility panel into topbar
  → On resource pages: config-{course}.js sets COURSE_CONFIG
  → course-viewer.js reads config, fetches CSV (live → snapshot fallback)
  → Renders topic list with filters, search, QR codes
```
...
base.html (head, meta, fonts, scripts, skip-link, <main>)
  ├── splitter.html (front door)
  ├── student_home.html (course cards)
  ├── teacher_home.html (teacher cards)
  ├── course_stub.html (per-course landing, student or teacher)
  ├── course_resources.html (resource viewer shell)
  ├── tools_hub.html (interactive tools)
  └── 404.html
```
